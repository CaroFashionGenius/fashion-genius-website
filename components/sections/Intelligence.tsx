...
export function Intelligence() { return <section className="section section-dark" id="intelligence"><div className="section-shell"><p className="section-kicker">THE INTELLIGENCE LAYER</p><div className="intro-grid"><h2 className="display">Data becomes<br/>the next action.</h2><p className="intro-copy muted">Fashion Genius combines product experience with operational intelligence — while the underlying agent, CRM and workflow architecture remains a protected system.</p></div><div className="intelligence-grid">{cards.map(([t,c],i)=><article key={t}><small>0{i+1}</small><h3>{t}</h3><p>{c}</p></article>)}</div></div></section> }
